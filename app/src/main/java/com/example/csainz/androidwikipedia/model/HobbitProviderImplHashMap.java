...
import com.example.csainz.androidwikipedia.model.CharacterContract.CharacterEntry;
...
/**
 * Content Provider implementation that uses a HashMap to manage Hobbit characters. This class
...
public class HobbitProviderImplHashMap extends HobbitProviderImpl  {
     ...
     */
    private static final HashMap<Long, CharacterRecord> mCharacterMap = new HashMap<>();
    ...
    public Uri insertCharacters(Uri uri, ContentValues cvs) {
        synchronized (this) {
            if (cvs.containsKey(CharacterEntry.COLUMN_NAME)) {
                CharacterRecord rec =
                    new CharacterRecord(cvs.getAsString(CharacterEntry.COLUMN_NAME),
                                        cvs.getAsString(CharacterEntry.COLUMN_RACE));
                mCharacterMap.put(rec.getId(), rec);
        ...
        if (selectionArgs == null) {
            final CharacterRecord updatedRec = 
                new CharacterRecord(rec.getId(),
                                    cvs.getAsString(CharacterEntry.COLUMN_NAME),
                                    cvs.getAsString(CharacterEntry.COLUMN_RACE));
            mCharacterMap.put(updatedRec.getId(), updatedRec);
            return 1;
        } else
    ...
    public int deleteCharacters(Uri uri,
        ...
        synchronized (this) {
            for (CharacterRecord cr : 
                     mCharacterMap.values()
                             .toArray(new CharacterRecord[mCharacterMap.values().size()]))
                recsDeleted += deleteEntryConditionally(cr,
                                                        selection,
                                                        selectionArgs);
        }
        return recsDeleted;
