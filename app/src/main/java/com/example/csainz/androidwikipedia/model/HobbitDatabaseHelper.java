...
/**
 * The database helper used by the Hobbit Content Provider to create and manage its underling
 * database.
 */
public class HobbitDatabaseHelper extends SQLiteOpenHelper {
    /**
     * Database name.
     */
    private static String DATABASE_NAME = "vandy_mooc_hobbit_db";
    ...
    final String SQL_CREATE_HOBBIT_TABLE = "CREATE TABLE "
     ...
     /**
     * Constructor - initialize database name and version, but don't actually construct the
     * database (which is done in the onCreate() hook method). It places the database in the
     * application's cache directory, which will be automatically cleaned up by Android if the
