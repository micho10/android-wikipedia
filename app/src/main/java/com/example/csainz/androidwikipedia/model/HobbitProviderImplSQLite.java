...
/**
 * Content Provider implementation that uses SQLite to manage Hobbit characters. This class plays
 * the role of the "Concrete Implementor" in the Bridge pattern and the "Concrete Class" in the
 * TemplateMethod pattern.
 */
public class HobbitProviderImplSQLite extends HobbitProviderImpl {
    /**
     * Use HobbitDatabaseHelper to manage database creation and version management.
     */
    ...
    public boolean onCreate() {
        ...
        mOpenHelper = new HobbitDatabaseHelper(mContext);
        return true;
    }
    ...
    public Uri insertCharacters(Uri uri, ContentValues cvs) {
        ...
        long id = db.insert(CharacterContract.CharacterEntry.TABLE_NAME, null, cvs);
    ...
    public int bulkInsertCharacters(Uri uri, ContentValues[] cvsArray) {
        ...
        try {
            for (ContentValues cvs : cvsArray) {
                final long id = db.insert(CharacterContract.CharacterEntry.TABLE_NAME, null, cvs);
                if (id != -1)
                    returnCount++;
            }
