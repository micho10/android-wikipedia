...
/**
 * Retains and manages state information between runtime configuration changes to an Activity.
...
public class RetainedFragmentManager {
    ...
    public boolean firstTimeIn() {
        try {
            // Find the RetainedFragment on Activity restarts. The RetainedFragment has no UI so
            // it must be referenced via a tag.
            mRetainedFragment = (RetainedFragment) mFragmentManager.get()
                    .findFragmentByTag(mRetainedFragmentTag);

            // A value of null means it's the first time in, so there's extra work to do.
            ...
            // A value of non-null means it's not first time in.
                ...
                Log.d(TAG, "Returning existing RetainedFragment " + mRetainedFragmentTag);
                return false;
            }
    ...
    /**
     * Return the Activity the RetainedFragment is attached to or null if it's not
     * currently attached.
     */
    ...
    /**
     * "Headless" Fragment that retains state information between configuration changes. Plays the
     * role of the "Memento" in the Memento pattern.
     */
